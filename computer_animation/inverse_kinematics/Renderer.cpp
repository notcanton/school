...
void Renderer::nanogui_init(GLFWwindow* window)
{
	...
	// NEW buttons: --------------------------------------------
	gui_1->setFixedSize(Eigen::Vector2i(100, 20));
	gui_1->addGroup("Rotation");
	bone_x1_widget = gui_1->addVariable("X1", m_bone_animation->X1);
	bone_y1_widget = gui_1->addVariable("Y1", m_bone_animation->Y1);
	bone_z1_widget = gui_1->addVariable("Z1", m_bone_animation->Z1);
	bone_x2_widget = gui_1->addVariable("X2", m_bone_animation->X2);
	bone_y2_widget = gui_1->addVariable("Y2", m_bone_animation->Y2);
	bone_z2_widget = gui_1->addVariable("Z2", m_bone_animation->Z2);
	bone_x3_widget = gui_1->addVariable("X3", m_bone_animation->X3);
	bone_y3_widget = gui_1->addVariable("Y3", m_bone_animation->Y3);
	bone_z3_widget = gui_1->addVariable("Z3", m_bone_animation->Z3);

	gui_1->addButton("Reset Bone Rotation", [&]() {
		m_bone_animation->reset();
		bone_x1_widget->setValue(0);
		bone_y1_widget->setValue(0);
		bone_z1_widget->setValue(0);
		bone_x2_widget->setValue(0);
		bone_y2_widget->setValue(0);
		bone_z2_widget->setValue(0);
		bone_x3_widget->setValue(0);
		bone_y3_widget->setValue(0);
		bone_z3_widget->setValue(0);
		bone_move->setValue(false);
		});
	

	bone_move = gui_1->addVariable("Bone Move: ", m_bone_animation->bone_move);
	...
	bone_xe_widget = gui_1->addVariable("Current end pos X", m_bone_animation->joints[3][0]);
	bone_ye_widget = gui_1->addVariable("Current end pos X", m_bone_animation->joints[3][1]);
	bone_ze_widget = gui_1->addVariable("Current end pos X", m_bone_animation->joints[3][2]);
	// ---------------------------------------------------------
...
void Renderer::draw_bones(Shader& shader, Bone_Animation* m_bone_animation)
{
	...
	// Draw the rest of the bones
	for (int i = 1; i < m_bone_animation->scale_vector.size(); i++) {
		root_bone_obj_mat = m_bone_animation->transform(i);
		glUniformMatrix4fv(glGetUniformLocation(shader.program, "model"), 1, GL_FALSE, glm::value_ptr(root_bone_obj_mat));
	...
	root_bone_obj_mat = glm::mat4(1.0f);
	root_bone_obj_mat = glm::translate(root_bone_obj_mat, m_bone_animation->target);
	glUniformMatrix4fv(glGetUniformLocation(shader.program, "model"), 1, GL_FALSE, glm::value_ptr(root_bone_obj_mat));
	...
	for (int i = 0; i <= 3; i++) {
		root_bone_obj_mat = glm::mat4(1.0f);
		root_bone_obj_mat = glm::translate(root_bone_obj_mat, m_bone_animation->joints[i]);
		glUniformMatrix4fv(glGetUniformLocation(shader.program, "model"), 1, GL_FALSE, glm::value_ptr(root_bone_obj_mat));
		bone_obj->obj_color = m_bone_animation->colors[i];
		draw_object(shader, *bone_obj);
	}
	*/
	...
	// Update GUI
	bone_x1_widget->setValue(m_bone_animation->X1);
	bone_x1_widget->setValue(m_bone_animation->X1);
	bone_y1_widget->setValue(m_bone_animation->Y1);
	bone_z1_widget->setValue(m_bone_animation->Z1);
	bone_x2_widget->setValue(m_bone_animation->X2);
	bone_y2_widget->setValue(m_bone_animation->Y2);
	bone_z2_widget->setValue(m_bone_animation->Z2);
	bone_x3_widget->setValue(m_bone_animation->X3);
	bone_y3_widget->setValue(m_bone_animation->Y3);
	bone_z3_widget->setValue(m_bone_animation->Z3);
	bone_move->setValue(m_bone_animation->bone_move);
	bone_xe_widget->setValue(m_bone_animation->joints[3][0]);
	bone_ye_widget->setValue(m_bone_animation->joints[3][1]);
	bone_ze_widget->setValue(m_bone_animation->joints[3][2]);
